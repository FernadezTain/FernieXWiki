...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function showProfile(avatar) {
    ...
    if (!profiles || !Object.keys(profiles).length) {
      showSimplePanel('Ошибка', 'Профили не загружены.');
      return;
    }
    ...
    if (!nick) {
      showSimplePanel('Профиль не найден', filename);
      return;
    }
    ...
    panel.querySelector('.open-profile-btn').onclick = () => {
      window.open(`profile.html?nick=${encodeURIComponent(nick)}`, '_blank');
    };
  }

  function showSimplePanel(title, msg) {
    alert(`${title}\n${msg}`);
  }
